...
![](https://codertw.com/wp-content/uploads/img/hMN68gCJm9.jpg)
...
#### 流量控制
tcp 接收双方都维护一个滑动窗口, ==接收方每次接收后回复响应时通过 tcp 首部的窗口字段(==缓存区的剩余大小称之为接收窗口大小==)来告诉发送方自己的窗口大小==, 发送方根据该值和其他信息来设置自己的窗口大小
...
- 401: unauthorized. 未授权(比如没登陆)
- 403: forbidden. 请求被拒绝(比如登陆了, 但角色权限不够)
- 404: not found
...
HTTPS劫持(抓包): 有可能被中间人攻击, 如 X 冒充 B, 给 A 发 X 的公钥, 这样 X 就能解密 A 发的密文

##### 证书
![](https://gblobscdn.gitbook.com/assets%2F-Me2HTYUybY4ojrYvI-8%2Fsync%2Fa368d636dbdd73001141acd6925221480f7ac8cb.jpg?alt=media)

现在正规的浏览器, 大都自带预存了正规认证机构的证书(包括其公钥)

##### https 连接过程
[参考](https://juejin.cn/post/6844903613479796750)
[参考](https://www.zhihu.com/question/52493697)
...
1. 机构确认身份(线上, 线下)后, 用自己的私钥 d2 签名 e1, 将 e1(还有签发时间, 域名, 上一级证书是否有效等), sign(机构的签名) 打包成证书发送给服务器
2. 浏览器向服务器发送请求, 同时发送一个随机数 client random
3. 服务器返回证书和机构的证书签名, 同时发送随机数 server random
4. 浏览器根据内置的机构公钥 e2 解密签名 sign 得到 e1', 与证书上的 e1 进行对比
5. 一致则确认服务器, 此时第三个随机数 pre-master key 有两种方法传输
    1. RSA, 使用服务端公钥加密后传输
    2. DH 算法, 双方协商得到 ([参考](https://blog.csdn.net/mrpre/article/details/52608867))利用到对数运算难以快速算出
6. 根据三个随机数得到 master key
7. 由 master key 推导出对称加密密钥 session key 和用于签名的 hash key
8. 通信时用HASH key 生成一个MAC（ Message Authentication Code），附在HTTP报文的后面，然后用session key 加密所有数据（HTTP + MAC），然后发送出去
9. 服务器先用session key 解密数据，得到HTTP + MAC，然后自己用相同的算法计算自己的MAC，如果两个MAC相等，则数据没有被篡改

- DH 算法的好处
前向安全, premaster key 从未出现在网络里, 即使后续密钥泄露, 也不能解出前面通信的内容
- 为什么需要三个随机数?
...
```
- session key 的作用
当会话突然中断需要恢复时, 可以利用 session id 得到 session key. 不需要再重新算出
##### https 的签名(如何保证不被篡改)
![](https://user-gold-cdn.xitu.io/2019/10/8/16daa659c3ac7cb0?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
![](https://pic3.zhimg.com/80/v2-012f7fe4ad010ef98048ec1286ca4747_1440w.jpg?source=1940ef5c)
1. 浏览器将发送报文的摘要1用自己的私钥 dA 进行签名后附在报文尾部
2. 服务器接收后, 用浏览器的公钥 eA 进行签名, 得到摘要1. 同时服务器对得到的明文进行hash, 得到摘要2
3. 比较摘要1和摘要2
